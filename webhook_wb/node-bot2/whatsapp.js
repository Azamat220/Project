...
// Telegram Bot tool for sending lead notifications (Clothing Store)
const telegramTool = {
  type: "function",
  function: {
    name: "send_telegram_notification",
    description: "Send customer lead information to Telegram group when they are ready to buy clothing",
    parameters: {
      type: "object",
      properties: {
        phone_number: {
...
        },
        product_interest: {
          type: "string",
          description: "The clothing item the customer is interested in (e.g., 'Summer Dress', 'Denim Jacket', 'Running Shoes')"
        },
        clothing_size: {
          type: "string",
          description: "The size of the item (e.g., 'XL', '42 EU', 'One Size', 'S')"
        },
        product_color: {
          type: "string",
          description: "The specific color of the item (e.g., 'Navy Blue', 'Beige', 'Black')"
        },
        design_style: {
          type: "string",
          description: "The fit or design style (e.g., 'Slim Fit', 'Oversized', 'V-neck', 'Casual')"
        }
      },
      required: ["phone_number", "delivery_address", "product_interest", "clothing_size", "product_color" ]
    }
  }
};
...
async function sendToTelegramGroup(leadData, userId) {
  try {
    ...
    const message = `*👗 НОВЫЙ ЗАКАЗ С WHATSAPP 👗*\n\n` +
                   `*🛍 Товар:* ${leadData.product_interest}\n` +
                   `—\n` +
                   `*📏 Размер:* ${leadData.clothing_size}\n` +
                   `*🎨 Цвет:* ${leadData.product_color}\n` +
                   `${leadData.design_style ? `*✂️ Фасон:* ${leadData.design_style}\n` : ""}` + 
                   `\n` +
                   `*📞 Телефон:* ${formatPhoneNumber(leadData.phone_number)}\n` +
                   `*📍 Адрес доставки:* ${leadData.delivery_address}\n\n` +
                   `🔗 [Написать в WhatsApp](${waLink})\n\n` +
                   `*⚠️ ТРЕБУЕТСЯ ДЕЙСТВИЕ:* Проверить наличие и оформить заказ!`;
...
function formatProductText(product) {
  if (product.name.toLowerCase().includes("qr")) {
    return `${product.description}`;
  } else {
    return `*${product.name}*\n${product.description ? `${product.description}\n` : ''}*Цена: ${product.price}*\n\nВам показать другие похожие модели или хотите заказать доставку?`;
  }
}
