  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        product_interest: {
          type: "string",
          description: "The product the customer is interested in (e.g., 'Kitchen chairs', 'Office desk')"
        },
        phone_number: {
          type: "string",
        ...
        delivery_address: {
          type: "string",
          description: "Delivery address provided by the customer"
        }
      },
      required: ["product_interest", "phone_number", "delivery_address"]
    }
  }
};
...
async function sendToTelegramGroup(leadData) {
  try {
    ...
    const formattedPhone = formatPhoneNumber(leadData.phone_number);
    ...
    const message = `🛋 *НОВАЯ ЗАЯВКА (Стулхан)*\n\n` +
                   `📦 *Интерес:* ${leadData.product_interest}\n` +
                   `📞 *Телефон:* \`${formattedPhone}\`\n` +
                   `📍 *Адрес:* ${leadData.delivery_address}\n\n` +
      ...
      try {
        ...
        if (toolCalls && toolCalls.length > 0) {
          ...
          for (const toolCall of toolCalls) {
            ...
            } else if (toolCall.function.name === 'send_order_to_telegram') {
		  try {
		    const args = JSON.parse(toolCall.function.arguments);
		    console.log('📤 Sending lead to Telegram:', args);

		    // Отправляем в Telegram
		    const telegramResult = await sendToTelegramGroup(args);
