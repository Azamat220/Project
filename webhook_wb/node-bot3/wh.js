...
// NEW: Define tool for sending Telegram notifications for HOT clients
const telegramTool = {
  type: "function",
  function: {
    name: "send_telegram_notification",
    description: "Send urgent notification to Telegram group when client is HOT (ready to buy immediately)",
    parameters: {
      type: "object",
      properties: {
        client_name: {
        ...
        product_interest: {
          type: "string",
          description: "Product or service the client is interested in"
        },
        urgency_level: {
          type: "string",
          description: "Level of urgency (HOT, URGENT, IMMEDIATE)"
        },
        budget: {
          type: "string",
          description: "Client's budget or price discussed"
        },
        timeline: {
          type: "string",
          description: "When client wants to purchase"
        },
        special_notes: {
          type: "string",
          description: "Any special notes about the client's readiness"
        }
      },
      required: ["client_name", "client_phone", "urgency_level"]
    }
  }
};

// NEW: Function to send notification to Telegram
async function sendTelegramNotification(notificationData, userId) {
  try {
    // Extract phone number from userId if not provided
    const phone = notificationData.client_phone || userId.split('@')[0];
    
	const message = `
	🔥 *ГОРЯЧИЙ КЛИЕНТ* 🔥

	*Клиент:* ${notificationData.client_name || 'Не указано'}
	*Телефон:* ${phone}
	*Интерес к продукту:* ${notificationData.product_interest || 'Не указано'}
	*Уровень срочности:* ${notificationData.urgency_level}
	*Бюджет:* ${notificationData.budget || 'Не указано'}
	*Сроки:* ${notificationData.timeline || 'Немедленно'}
	*Особые примечания:* ${notificationData.special_notes || 'Нет'}

	*Статус:* ГОТОВ 🚀
	*Время:* ${new Date().toLocaleString()}

  *ТРЕБУЕТСЯ ДЕЙСТВИЕ:* Свяжитесь немедленно!
`.trim();

    console.log(`📱 [TELEGRAM] Sending HOT client notification:`, {
      client: notificationData.client_name,
      phone: phone,
      urgency: notificationData.urgency_level
    });
    ...
    const response = await fetch(`https://api.telegram.org/bot${telegramBotToken}/sendMessage`, {
      ...
      body: JSON.stringify({
        ...
        text: message,
        parse_mode: 'Markdown',
        disable_notification: false // Important: make sure notification is enabled for HOT clients
      })
    });
...
export default function initializeWhatsAppClient(appState) {
  ...
  async function processUserBatch(userId, appState, client) {
    ...
    try {
        ...
        for (const msg of resolvedMessages) {
          try {
            await dbClient.query(
              `INSERT INTO conversations_2 (user_id, role, message, whatsapp_msg_id, media_type, bot_id)
      ...
      // Call OpenAI API with tool support (including new telegramTool)
      try {
        const openai = new OpenAI({apiKey: process.env.OPENAI_API_KEY});
        const completion = await openai.chat.completions.create({
          model: "gpt-4o",
          messages,
          tools: [productTool, amoTool, telegramTool], // ADDED: telegramTool
          tool_choice: "auto",
          max_tokens: 1000,
          temperature: 0.7
        });
        ...
        if (toolCalls && toolCalls.length > 0) {
          ...
          for (const toolCall of toolCalls) {
            if (toolCall.function.name === 'get_product_info') {
              ...
              try {
                const args = JSON.parse(toolCall.function.arguments);
                console.log(`🔥 Sending HOT client notification to Telegram: ${JSON.stringify(args)}`);
                // Send Telegram notification
                telegramResult = await sendTelegramNotification(args, userId);
                // Add tool response to messages
                messages.push({
                  tool_call_id: toolCall.id,
                  role: "tool",
                  name: "send_telegram_notification",
                  content: JSON.stringify(telegramResult)
                });

                if (telegramResult) {
                  console.log('✅ Telegram notification sent successfully for HOT client');
                } else {
                  console.log('❌ Failed to send Telegram notification');
                }
              } catch (err) {
                console.error('❌ Telegram tool call error:', err);
                messages.push({
                  tool_call_id: toolCall.id,
                  role: "tool",
                  name: "send_telegram_notification",
        ...
        try {
          await db.query(
            `INSERT INTO conversations_2 (user_id, role, message, bot_id)
        ...
        try {
          if (appState.clientReady) {
            const errorMsg = await client.sendMessage(
              userId,
              "Извините, у меня возникли проблемы с обработкой вашего запроса. Пожалуйста, попробуйте еще раз."
            );
            if (errorMsg && errorMsg.id && errorMsg.id._serialized) {
              appState.botMessageIds.set(errorMsg.id._serialized, Date.now());
            }
          }
...
// 1. Сначала получаем контакт и чистый номер
  ...
  // Human agent detection - MODIFIED: Only activate after second message from manager
  client.on('message_create', (msg) => {
    ...
    // Check if bot is sending this message
    if (appState.botSendingMessage) {
      appState.botMessageIds.set(msgId, Date.now());
      return;
    }

    // Check if this is a known bot message
    ...
    // Human agent handling - NEW LOGIC: Only activate after second message
    if (customerId && customerId.includes('@c.us')) {
      // Get current message count for this chat
      const currentCount = appState.managerMessageCounts.get(customerId) || 0;
      const newCount = currentCount + 1;
      appState.managerMessageCounts.set(customerId, newCount);

      console.log(`👨‍💼 Manager message #${newCount} in chat with ${customerId}`);

      // Only activate human chat on second or subsequent message
      if (newCount >= 2) {
        console.log(`👨‍💼 Human agent activated in chat with ${customerId} (message #${newCount})`);
        appState.activeHumanChats.add(customerId);

        // Clear existing timer
        if (appState.chatTimers.has(customerId)) {
          clearTimeout(appState.chatTimers.get(customerId));
        }

        // Set new timer
        const timer = setTimeout(() => {
          appState.activeHumanChats.delete(customerId);
          appState.chatTimers.delete(customerId);
          // Reset message count when chat is released
          appState.managerMessageCounts.delete(customerId);
          console.log(`🕒 Auto-released chat with ${customerId} after ${appState.autoReleaseTimeout/60000} minutes`);
        }, appState.autoReleaseTimeout);

        appState.chatTimers.set(customerId, timer);
      } else {
        console.log(`⏳ First manager message to ${customerId} - bot remains active`);
        // For first message, don't activate human chat - bot continues to respond
      }
    }
